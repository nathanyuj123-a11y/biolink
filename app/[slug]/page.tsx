...
export default async function BioPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="relative min-h-[100svh] overflow-hidden bg-[#0d0d0d]">
      ...
      <main className="relative z-10 mx-auto flex min-h-[100svh] w-full max-w-[420px] flex-col px-5 pb-6 pt-5">
        {/* Logo + cidade compactos */}
        <div className="flex flex-col items-center">
          <img
            src="/logos/logo-white.png"
            alt="S.A Franchising"
            className="w-full max-w-[260px] h-auto object-contain drop-shadow-[0_10px_25px_rgba(0,0,0,0.6)]"
          />
          <h1 className="font-bebas text-3xl leading-none tracking-[0.04em] text-neutral-100 mt-3">
            {store.cidade}
          </h1>
          <div className="mt-1.5 flex items-center gap-2 text-neutral-500">
            <span className="h-px w-4 bg-neutral-700" />
            <span className="font-mono text-[9px] tracking-[0.35em] text-sared">{store.uf}</span>
            <span className="h-px w-4 bg-neutral-700" />
          </div>
        </div>
          ...
          <div className="mt-5 space-y-2">
            {deliverys.map((c) => (
              <a
                ...
                target="_blank"
                rel="noopener"
                className="group relative flex w-full items-center justify-between overflow-hidden rounded-xl bg-sared px-4 py-3 transition-all duration-300 hover:bg-[#a30d24] hover:shadow-[0_8px_20px_-8px_rgba(200,16,46,0.55)]"
              >
                <span className="absolute inset-y-0 left-0 w-px bg-white/20" />
                <div className="text-left min-w-0">
                  <p className="font-mono text-[8px] uppercase tracking-[0.35em] text-white/70">
                    Quero pedir agora
                  </p>
                  <p className="font-bebas text-lg tracking-[0.05em] text-white leading-none mt-0.5 truncate">
                    Delivery {c.label}
                  </p>
                </div>
                <svg
        ...
        {store.google_review_url && (
          <a
            ...
            target="_blank"
            rel="noopener"
            className="group mt-3 flex w-full items-center justify-between rounded-xl border border-amber-500/30 bg-amber-500/[0.06] px-4 py-3 transition-all duration-300 hover:border-amber-400/60 hover:bg-amber-500/[0.12]"
          >
            <div className="text-left flex items-center gap-2.5 min-w-0">
              <svg width="18" height="18" viewBox="0 0 24 24" className="shrink-0 text-amber-400" fill="currentColor">
                <path d="M12 2l2.4 7.4H22l-6.2 4.5 2.4 7.4L12 16.8 5.8 21.3l2.4-7.4L2 9.4h7.6z" />
              </svg>
              <div className="min-w-0">
                <p className="font-mono text-[8px] uppercase tracking-[0.35em] text-amber-300/70">
                  Sua opinião conta
                </p>
                <p className="font-bebas text-base tracking-[0.05em] text-amber-100 leading-none mt-0.5 truncate">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <a
            ...
            target="_blank"
            rel="noopener"
            className="flex flex-col items-center justify-center gap-1 rounded-xl border border-neutral-800 bg-neutral-950/60 px-3 py-2.5 transition-all duration-300 hover:border-neutral-600 hover:bg-neutral-900"
          >
        ...
        {/* Footer no fim do flex */}
        <div className="mt-auto pt-5 flex items-center justify-center gap-3 text-neutral-700">
          <span className="h-px w-6 bg-neutral-800" />
          <span className="font-mono text-[8px] uppercase tracking-[0.45em]">
            S.A Franchising
          </span>
          <span className="h-px w-6 bg-neutral-800" />
        </div>
